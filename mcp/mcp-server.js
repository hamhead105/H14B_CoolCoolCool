...
function createMcpServer() {
  ...
  const authCache = {
    buyer: null,   // { buyerId, token, email }
    seller: null,  // { sellerId, token, email }
  };
  ...
  server.tool(
    ...
    async ({ email, password }) => {
      ...
      authCache.buyer = { buyerId: data.buyerId, token: data.token, email };
      return result({ buyerId: data.buyerId, token: data.token, cached: true });    }
  );
  ...
  server.tool(
    ...
    async ({ email, password }) => {
      ...
      if (!ok) return err(`Login failed: ${data.error || data.message || 'unknown error'}`);
      authCache.seller = { sellerId: data.sellerId, token: data.token, email };
      return result({ sellerId: data.sellerId, token: data.token, cached: true });
    }
  );
  ...
  server.tool(
    'get_cached_token',
    'Retrieve a cached buyer or seller token from this session. Use this before re-logging in — if a token is cached, use it directly.',
    { role: z.enum(['buyer', 'seller']).describe('Which role token to retrieve') },
    async ({ role }) => {
        const cached = authCache[role];
        if (!cached) return err(`No cached ${role} token. Please login first.`);
...
server.tool(
  'get_schema_guide',
  'ALWAYS call this tool first before any other tool in a new conversation. Returns the full API reference, Prisma schema, and workflow rules.',
  {},
  async () => ({
    content: [{
      type: 'text',
      text: `
CoolCoolCool API — Full Reference for Claude
============================================

⚠️  READ THIS BEFORE CALLING ANY OTHER TOOL ⚠️

═══════════════════════════════════════════
AUTHENTICATION
═══════════════════════════════════════════
- Call buyer_login OR seller_login first. Save the returned token + id.
- Use get_cached_token to reuse a token within the same session.
- On any auth error → re-login, do not retry with the same token.

═══════════════════════════════════════════
PRISMA MODEL: Product
═══════════════════════════════════════════
All fields are REQUIRED when calling create_product:

  productId    String   — Auto-generated by MCP server. NEVER pass this in.
  name         String   — Product name. Required.
  description  String   — Must be non-empty. Generate one if not provided.
  cost         Float    — Base price in AUD (before discount).
  brand        String   — Brand name. Required.
  family       String   — Product category e.g. "Medicine", "EPhone", "Honey".
  onSpecial    Boolean  — Default: false.
  discount     Float    — Decimal 0.0–1.0 (NOT percentage). e.g. 0.15 = 15% off.
                          Only meaningful when onSpecial = true. Default: 0.
  productTier  Int      — 1 = standard, 2 = premium, 3 = exclusive. Default: 1.
  nextProduct  String   — productId of upgraded model in same line. Use "" if none.
  sellerId     Int      — From seller_login response.

  effectiveCost = onSpecial ? cost * (1 - discount) : cost

═══════════════════════════════════════════
PRISMA MODEL: Buyer
═══════════════════════════════════════════
  buyerId       Int      — Auto-incremented PK.
  name          String
  email         String   — Unique.
  street        String
  city          String
  ...
  taxSchemeId   String?  — Optional. Default "GST".
  contactPhone  String?  — Optional.
  loyaltyPoints Int      — Default 0. 100 pts = $1 AUD discount.

═══════════════════════════════════════════
PRISMA MODEL: Seller
═══════════════════════════════════════════
  sellerId      Int      — Auto-incremented PK.
  name          String
  email         String   — Unique.
  street        String
  city          String
...
PRISMA MODEL: Order
═══════════════════════════════════════════
  orderId                  String   — Auto-generated (ORD-<timestamp>).
  buyerId                  Int?
  status                   String   — e.g. "order placed", "despatched", "invoiced", "partially fulfilled"
  totalCost                Float
  taxAmount                Float    — 10% GST applied server-side.
  payableAmount            Float    — totalCost + taxAmount - loyaltyDiscount
  loyaltyPointsEarned      Int
  loyaltyPointsRedeemed    Int

  create_order items must include:
    productId, name, description, quantity,
    effectivePrice (post-discount price, NOT base cost),
    sellerId, onSpecial

═══════════════════════════════════════════
WORKFLOW — Seller: Photo → Product Catalogue
═══════════════════════════════════════════
1. seller_login → save token + sellerId
2. Extract product details from image/document
3. Confirm ALL fields with user (name, brand, family, cost, tier, description)
4. create_product for each item — pass ALL required fields
5. list_products_for_seller to confirm catalogue

═══════════════════════════════════════════
WORKFLOW — Buyer: Minimise My Cart
═══════════════════════════════════════════
1. buyer_login → save token + buyerId
2. search_products or get_product_family to find cheapest options
3. get_loyalty_points → decide whether to redeem
4. Confirm order details with user
5. create_order with effectivePrice (not base cost), redeemLoyaltyPoints
6. generate_invoice to complete the cycle

═══════════════════════════════════════════
COMMON MISTAKES TO AVOID
═══════════════════════════════════════════
- discount must be 0.0–1.0 decimal, NOT 0–100 percentage
- Never pass productId into create_product — it's generated server-side
- Always pass effectivePrice (discounted) into create_order, not cost
- description must never be empty in create_product
- nextProduct must be "" (empty string) if there is no next product
- Tokens expire — if you get an auth error, re-login immediately
      `.trim(),
    }],
  })
);
  ...
  server.tool(
    'create_product',
    'Create a new product listing for a seller create feilds that arent provided other than important feilds like price. Use this after extracting product details to onboard a catalogue.',
    {
  ...
  server.tool(
    'list_products_for_seller',
    "List all products belonging to a specific seller. Use after bulk-creating products to confirm the catalogue was created correctly. Seller may already have products from before — this lists them all, not just the new ones.",
    {
      token:    z.string().describe('JWT token'),
  ...
  server.tool(
    'update_product',
    'Update an existing product listing. Use to correct details after creating products.',
    {
      token:       z.string().describe('JWT token from seller_login'),
